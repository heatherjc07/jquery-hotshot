$(function () {
    ...
    //variables
    var numberOfPieces = 48,
        aspect = "6:8", //3 across, 4 down
    ...
    //remove image
    img.remove();
    //remove top left piece note: make sure it isn't key
    container.find("#0").remove();

    //also remove from positions array
    positions.shift();
    ...
    $("#start").on("click", function (e) {
        ...
        pieces.draggable({
            ...
            start: function (e, ui) {
                ...
                if (current.left === empty.left) {
                    ui.helper.draggable("option", "axis", "y"); // move on y axis
                } else if (current.top === empty.top) {
                    ui.helper.draggable("option", "axis", "x"); // move on x axis
                } else {
                    ui.helper.trigger("mouseup"); //can't move
                    return false;
                }

                //prevent drag if not adjacent to empty space
                if (current.bottom < empty.top || current.top > empty.bottom 
				     || current.left > empty.right || current.right < empty.left) {
            ...
            drag: function (e, ui) {
                ...
                //stop dragging if in empty space
                ...
                if (current.top > empty.bottom || current.bottom < empty.top 
				     || current.left > empty.right || current.right < empty.left) {
            ...
            stop: function (e, ui) {
                ...
                if (correctPieces === positions.length) {
					...
					//see modernizer on github for workarounds for old browsers
        ...
        //helper to get position of current piece
